...
a separate `resume` verb, a `replay` field for a resumed transcript, and `Promise`-wrapped handlers
...
- The session snapshot no longer carries an id, so `isCurrentSnapshot` validates an old blob by its
  node and block log alone.
