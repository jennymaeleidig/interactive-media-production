...
- **Take-over**: full-screen `bg-paper` panel, hairline-divided flat list (PlumeOS, Trust — serif-300 rows, no chevrons), full-width pill "Book a demo" CTA after the list, `min-h-[100dvh]`. Layered beneath the assistant widget — the assistant is always on top (map standing decision; ticket 02 formalized the z-index tokens, which renumbered these layers: take-over 30, nav 40, assistant 50).
