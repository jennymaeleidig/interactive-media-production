...
- [Mobile type scale fills the screen](issues/04-mobile-type-scale.md): root scale re-floored to ~14px at 390px (16px at desktop, fluid kept), display clamp floors 3rem→3.25rem plus a max-md boost on the fixed mid-H2 tier (~44px+ at 390px), and a fluid --text-body token (15px→16px) so body copy and micro voices read comfortably on phones; nothing overflows at 320px, z-index tokens untouched.
