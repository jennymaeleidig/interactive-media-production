...
- [Chat wrapper prototype](issues/04-chat-wrapper-prototype.md): server-side Dialogue runtime — SSR `loadYarnProject()` startup-singleton, zero bundler config (vite plugin rejected: Turbopack has no seam); npm `file:` dep on the sibling via absolute path (publish later); message API `{start|resume|option}` → turn batch + `vars` = `VariableStorage.entries()` per turn; **all choices are yarn options rendered as user-style bubbles in the composer slot** — no composer, no free text, no persistent chips (user direction); prototype at `prototypes/chat-wrapper/` (throwaway, runnable). Env facts: `WATCHPACK_POLLING=true` for dev, `browserslist` field required, yarn comments between options silently drop following options.
