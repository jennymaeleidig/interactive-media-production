...
- **Effort layout**: ticket bodies in `issues/`; research briefs for spawned agents in `briefs/`; full research findings in `research/` (each ticket's `## Answer` stays a short gist linking to its research file); capture runs in `research/flocksafety/`.
