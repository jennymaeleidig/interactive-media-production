...
- One assertion flipped in `logs every mutation per page`: `bytesIn > bytesOut` no longer holds on the miniature fixture (the 2.6KB runtime outweighs what the tiny fixture loses to stripping); real pages still shrink ~2MB each — the mutation log's strip counts carry that, and the comment points at the real-run spot-check.
