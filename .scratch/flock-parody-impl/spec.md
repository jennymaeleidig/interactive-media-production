...
- The seams, in order of height:
  ...
  The ideal is one seam; the pixel gate is that whole-system seam — the others exist because the chat (server state) and the DOM-contract runtimes (story-hook, motion, interactions) are genuinely separate surfaces.
