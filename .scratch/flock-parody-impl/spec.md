...
- A changed page invalidates exactly its own Recreation route: re-run the five passes on that page from the fresh Capture, then its serving check. The capture pointer (which run folder the build serves from) is a single config value. Captures remain out of git; inventories, CSVs, and markdown stay tracked.
...
- The prototype assets live with the effort and are throwaway; the liftable parts are the five-pass pipeline script, the motion CSS/runtime pair, and the chat engine module.
