...
//
// **The capture itself is gone.** The run lived under `.scratch/` — 34.5 GB of
// captured HTML, gitignored because heavy HTML never belonged in git — and the
// whole scratch tree was scrapped when the effort closed (2026-09-13). The
// served tree it produced is kept; this pointer now names where a future run
// would live. `npm run pipeline` therefore cannot run until one does: it needs
// the captures. The serving check does not (`CAPTURE_LIST` below).
...
// from — a record, which is exactly what a scrapped run can no longer supply.
// Copy of `.scratch/flock-parody/research/flocksafety/2026-09-12/capture-list.txt`,
// taken before the deletion and never regenerated by the build.
...
// `research/video-inventory/2026-09-11/playability.csv`, which
