 ...
 * `name`/`json`/`stamp` only: the page each layer is scoped to, and the path its
 * generated bytes live at, belong to the roster — this script reads them from
 * `LAYERS` (`rosterEntry`) rather than restating them, so a page move or a rename
 * edits the roster once and the build follows.
 * @type {{ name: string, json: string, stamp: string }[]}
 */
const HEROES = [
  { name: 'flock-dfr', json: 'flock-dfr.json', stamp: '697aa404c8693265548d6485_' },
  { name: 'flock-freeform', json: 'flock-freeform.json', stamp: '686287b72c241200cdd5044f_' },
  { name: 'flock-os', json: 'flock-os.json', stamp: '685d8d69194804a7cdd5c666_' },
  { name: 'flock-safety-platform', json: 'flock-safety-platform.json', stamp: '68618f0085bf4baa1110082c_' },
  { name: 'gunshot-detection', json: 'gunshot-detection.json', stamp: '685bcd69fc6a6e0809525986_' },
  { name: 'video-cameras', json: 'video-cameras.json', stamp: '68618f027169b5bd9b1f59b1_' },
];

/**
 * The roster entry a hero's bytes belong to, and the page it is scoped to — the
 * one place a layer's route and source path are written down. Fails loudly when
 * the roster stops declaring the layer, scopes it to no page, or gives it no js
 * source: this script must not invent what the roster owns.
 * @param {{ name: string }} hero
 * @returns {{ route: string, source: string }}
 */
function rosterEntry(hero) {
  const name = `lottie-${hero.name}`;
  const layer = LAYERS.find((candidate) => candidate.name === name);
  if (!layer) throw new Error(`${name}: the roster declares no such layer`);
  const [route] = layer.pages ?? [];
  if (!route) throw new Error(`${name}: the roster scopes it to no page`);
  const source = layer.parts.find((part) => part.kind === 'js')?.source;
  if (!source) throw new Error(`${name}: the roster declares no js source`);
  return { route, source };
}
...
function main() {
  ...
  for (const hero of HEROES) {
    const { route, source } = rosterEntry(hero);
    ...
    const out = path.join(ROOT, source);
    writeFileSync(out, runtime, 'utf8');
    const hash = createHash('sha256').update(runtime).digest('hex').slice(0, 16);
    console.log(
      `  ${hero.name.padEnd(24)} exprs=${String(total).padStart(2)}  ${String(runtime.length).padStart(9)} chars  sha256_16=${hash}  → ${source}  (${route})`,
    );
  }
}
