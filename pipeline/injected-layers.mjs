...
// `story-hook`, `scroll`) and one page-scoped patch (`legibility`, only on
// `/safe-cities`). Each is injected into a served page carrying
...
export const LAYERS = [
  ...
  },
  {
    ...
    parts: [
      ...
      // the one member allowed to reach anything, and only the local message
      // API: one POST per turn IS the chat mount (`injected-source.mjs`, and
      // the CSP's `connect-src 'self'` is what admits it)
      { kind: 'js', delivery: 'asset', source: 'pipeline/chat-widget.js', outbound: 'self' },
    ],
  },
  {
    name: 'story-hook',
