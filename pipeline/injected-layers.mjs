...
// `/safe-cities`), and the Lottie layers — the shared `lottie-player` and the six
// per-page `lottie-<name>` data runtimes that use it. Each is injected into a served page carrying
...
export const LAYERS = [
  ...
  // The shared Lottie player: one asset for all six hero pages, ahead of the data
  // runtimes that use its `window.lottie` — the position is the load order. It is
  // ~300 KB of identical bytes that used to be inlined into each of the six, and
  // it is `insert: true` because it is a tag the publisher creates rather than one
  // the capture ever carried.
  //
  // The six Lottie data runtimes, one per product page, in the order they ship.
  // Each is generated by `pipeline/build-lottie-layers.mjs`, which reads this
  // roster for the pages and the source paths and holds only the vendored JSON
  // and its stamps. They carry no `outbound` because the embedded animation data
  // is all `data:` URIs and the neuter names no network primitive
  // (`injected-source.mjs`).
  //
  ...
  // the publisher could write one, and it can now recreate them byte-for-byte.
  {
    name: 'lottie-player',
    scope: 'page',
    pages: LOTTIE_HEROES.map((name) => `/products/${name}`),
    parts: [{ kind: 'js', delivery: 'asset', source: 'pipeline/lottie/player.runtime.js', insert: true }],
  },
