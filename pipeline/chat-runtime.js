...
(() => {
  ...
  function runUntilStopped(dialogue, prior = EMPTY_TRANSCRIPT) {
    const { events, stopped } = pullUntilStopped(dialogue);
    if (events.length === 0) {
      return { transcript: prior, stopped };
    }
    const atRest = prior.options !== null ? __spreadProps(__spreadValues({}, prior), { options: null }) : prior;
    return { transcript: mergeEvents(events, atRest), stopped };
  }
  function mergeEvents(events, prior = EMPTY_TRANSCRIPT) {
    var _a;
    let lines = null;
    let options = null;
    let commands = null;
    ...
    for (const event of events) {
      if (event.type === "line") {
        (lines != null ? lines : lines = [...prior.lines]).push({
          lineId: event.lineId,
        ...
        (commands != null ? commands : commands = [...prior.commands]).push(event.command);
      } else if (event.type === "nodeStart") {
        scene = (_a = event.scene) != null ? _a : scene;
      }
    }
    ...
    return {
      lines: lines != null ? lines : prior.lines,
      options: options != null ? options : prior.options,
      commands: commands != null ? commands : prior.commands,
      scene
    };
  }
  ...
  function rebuild(snapshot) {
    ...
    if (!snapshot.complete && snapshot.node) {
      ...
      pendingOptions = stopped === "options" ? (_a = transcript.options) != null ? _a : null : null;
  ...
  function start(sessionId) {
    ...
    const pending = stopped === "options" ? (_a = transcript.options) != null ? _a : [] : null;
  ...
  function option(sessionId, optionIndex) {
    ...
    log.push(...lines);
    const complete = stopped === "complete";
    const nextPending = stopped === "options" ? (_b = transcript.options) != null ? _b : [] : null;
