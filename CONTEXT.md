...
- **Strip decision** — what the build pass removed from each Capture (Qualified offer host and header-height var, the chat launcher, the OneTrust consent stack), recorded per page in the build log with removed byte counts; the human side-by-side at the phase gates reviews the visual result.
