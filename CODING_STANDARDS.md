...
- Tests assert **external behavior only**, at the seams the tests pin:
  ...
  - **The chat dialogue seam** (`test/chat.seam.test.ts`) — the conversation the
    ...
    jsdom its one `localStorage` session needs: pinned copy, turn shapes, session
    persistence, and the email-gate branch. There is no HTTP boundary to drive —
    the message API and its `POST /api/chat` shell are retired.
