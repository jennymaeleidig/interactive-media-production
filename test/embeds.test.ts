...
// snapshot for the live player document. Pure, so it is pinned here rather than
// only through the site build; the allow-list audit that keeps that exception
// from spreading lives in `pipeline/audit.mjs` and is pinned in
// `test/audit.test.ts`.
//
