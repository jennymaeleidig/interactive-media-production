// Chat dialogue seam: the engine the tree ships, driven headlessly.
...
// `pipeline/build-chat-runtime.mjs` bundles into the committed `chat-runtime.js`
// asset every served page carries — run in a real DOM (jsdom) because its one
// session lives in `localStorage`, not in a server process. There is no HTTP
// boundary to drive: the message API and its `POST /api/chat` shell are
// retired, so this is the surface that ships.
//
